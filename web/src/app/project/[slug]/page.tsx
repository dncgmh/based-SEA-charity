    ...
    <div className='container mx-auto animate-fadeIn p-4'>
      ...
      <div className='grid gap-8 lg:grid-cols-3'>
        <div className='lg:col-span-2'>
          <div className='group relative'>
            <Image src={project.images[0]} alt={project.title} width={800} height={600} className='h-[400px] w-full rounded-lg object-cover shadow-lg' />
          ...
          <div className='card sticky top-4 bg-base-100 shadow-xl'>
            <div className='card-body'>
              ...
              <div className='mb-4 h-2.5 w-full rounded-full bg-gray-200 dark:bg-gray-700'>
                <div className='h-2.5 rounded-full bg-blue-600 transition-all duration-1000 ease-out' style={{ width: `${progressPercentage}%` }} />
              </div>
              <div className='flex justify-between font-medium text-sm'>
                <span> {formatCurrency(totalFundsRaisedValue)} USD raised</span>
                <span>{formattedProgressPercentage}%</span>
              </div>
              <p className='mt-1 text-right text-gray-600 text-sm'>of {formatCurrency(goal, { fractionDigits: 0 })} goal</p>
          ...
          <div className='carousel carousel-center space-x-4 rounded-box'>
            {project.images.map((image, index) => (
              <div key={index} className='carousel-item'>
                <Image src={image} alt={`${project.title} - Image ${index + 1}`} width={150} height={150} className='rounded-lg shadow-md' />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className='mt-6 flex flex-wrap gap-2'>
        {project.tags.map((tag, index) => (
          <div key={index} className='badge badge-outline cursor-pointer p-3 transition-all duration-300 hover:bg-info hover:text-white'>

...
export default function OpenDonationsModal({ project, onProjectUpdate }) {
  ...
  const { deployContract, data: hash, isPending, isSuccess: isDeploySuccess, error: deployError, reset: resetDeployContract } = useDeployContract({});
  ...
  return (
    <dialog id='open-donations' className='modal'>
      <div className='modal-box'>
        ...
        <p className='py-4'>Opening donations will deploy a smart contract. Please note that this will require gas fees.</p>
        ...
        <div className='modal-action'>
          <form method='dialog'>
            {!isTxSuccess && (
              <button disabled={isPending || isDeploying} type='button' className='btn btn-sm h-full' onClick={onOpenDonations}>
